...
export function InventoryItemTile({
  ...
  const { sceneProps, showingBack, flip } = useCardFlip({ enabled: hasBothImages });
    ...
    <div className="flex flex-col gap-2 rounded-lg bg-card p-2">
      <div
        ...
        className={cn(
          ...
          hasBothImages && "flip-card-swipeable",
      ...
      >
        ...
        {hasBothImages && <div className="flip-card-sheen" aria-hidden />}

        {item.custom_image_url && (
          <Badge variant="secondary" className="absolute bottom-1 left-1">
            {showingBack ? "Stock image" : "Your photo"}
          </Badge>
        )}
        ...
        {hasBothImages && (
          <Button
            ...
            variant="ghost"
            onClick={flip}
            aria-label={showingBack ? "Show your photo" : "Show stock image"}
      ...
      <div className="flex flex-col gap-0.5">
        ...
        {item.card.set_name && (
          <p className="truncate text-[11px] text-muted-foreground/70">{item.card.set_name}</p>
        )}
